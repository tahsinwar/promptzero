...
import { Save, ArrowLeft, Plus, Trash2, Copy as CopyIcon, X, Loader2, Share2, Globe, EyeOff, ChevronUp, ChevronDown, Info, AlertTriangle, GripVertical, Wand2, Undo2, Bold, Italic, Heading1, Heading2, Code, Code2, Link as LinkIcon, List, ListOrdered, Quote, Strikethrough, HelpCircle, History } from "lucide-react";
      ...
      <div className="flex flex-wrap items-center gap-1 border-b border-border px-1.5 py-1">
        ...
        <button
          ...
          className={`inline-flex items-center gap-1 rounded border px-2 h-7 text-[11px] font-medium ${
            showHelp ? "border-primary/40 bg-primary/10 text-primary" : "border-border bg-background/40 text-muted-foreground hover:text-foreground"
          }`}
        >
          <HelpCircle className="h-3.5 w-3.5" /> Help Notes
  ...
  return (
    <div className="max-h-[320px] overflow-auto px-3 py-2">
      <div className="flex items-center gap-2 pb-2">
        ...
        <h4 className="text-xs font-bold uppercase tracking-wider text-foreground/80">Notes — Markdown cheatsheet</h4>
      </div>
      ...
      <p className="mt-2 text-[11px] text-muted-foreground">HTML tags are stripped for safety. Only <span className="font-mono">http(s)/mailto/tel</span> links allowed.</p>
    </div>
  );
}
