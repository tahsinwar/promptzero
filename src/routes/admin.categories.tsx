          ...
          <div key={c.id} className="vault-card rounded-lg p-3 flex items-center gap-3">
            {editId === c.id ? (
              <>
                ...
                <button disabled={update.isPending} onClick={() => update.mutate()} className="rounded-md bg-primary/15 text-primary p-2 disabled:opacity-60">{update.isPending ? <Loader2 className="h-4 w-4 animate-spin" /> : <Check className="h-4 w-4" />}</button>
              ...
              <>
                ...
                <button disabled={remove.isPending} onClick={() => confirm(`Delete ${c.name}?`) && remove.mutate(c.id)} className="rounded-md p-2 text-muted-foreground hover:text-destructive hover:bg-secondary disabled:opacity-60"><Trash2 className="h-4 w-4" /></button>
              </>
            )}
          </div>
        ))}
  ...
  return (
    <section>
      ...
      >
        ...
        <button disabled={create.isPending} className="inline-flex items-center gap-2 rounded-lg bg-primary px-4 py-2 text-sm font-semibold text-primary-foreground disabled:opacity-60">
          {create.isPending ? <Loader2 className="h-4 w-4 animate-spin" /> : <Plus className="h-4 w-4" />}Add tag
        </button>
      </form>

      {isLoading && !data ? <AdminListSkeleton rows={2} /> : (
      <div className="flex flex-wrap gap-2">
        {data?.map((t) => (
          <span key={t.id} className="inline-flex items-center gap-2 rounded-full border border-border bg-card px-3 py-1.5 text-sm">
            <span>#{t.name}</span>
            <button disabled={remove.isPending} onClick={() => remove.mutate(t.id)} className="text-muted-foreground hover:text-destructive disabled:opacity-60" aria-label={`Remove ${t.name}`}>
